...
/**
 * 🚨🚨🚨 GLOBAL DATA CHANNEL MANAGEMENT SYSTEM 🚨🚨🚨
 * 
 * This system prevents the critical bug where incoming channels overwrite outgoing channels
 * on recipient side, causing messages to fail sending.
 * 
 * Key principles:
 * - On SENDER side: Incoming channel (from recipient) REPLACES outgoing channel (for receiving messages)
 * - On RECIPIENT side: Outgoing channel (for sending) MUST NEVER be overwritten by incoming channel
 * - Channels are stored with metadata to track their direction (outgoing/incoming)
 */
window.TelecomChannelManager = (() => {
  // Track channel direction and role
  const channelMetadata = new Map(); // contactGuid -> { outgoing: channel, incoming: channel, role: 'sender'|'recipient' }
  
  /**
   * Store outgoing channel (created by createDataChannel)
   * @param {string} contactGuid - Contact GUID
   * @param {RTCDataChannel} channel - Outgoing data channel
   * @param {string} role - 'sender' or 'recipient'
   */
  function storeOutgoingChannel(contactGuid, channel, role) {
    if (!window._telecomDataChannels) {
      window._telecomDataChannels = new Map();
    }
    
    const metadata = channelMetadata.get(contactGuid) || { role };
    metadata.outgoing = channel;
    metadata.role = role;
    channelMetadata.set(contactGuid, metadata);
    
    // Store in main map (for backward compatibility)
    window._telecomDataChannels.set(contactGuid, channel);
    
    console.log(`[ChannelManager] ✅ Stored OUTGOING channel for ${role}:`, {
      contactGuid,
  ...
  /**
   * Store incoming channel (received via ondatachannel)
   * @param {string} contactGuid - Contact GUID
   * @param {RTCDataChannel} channel - Incoming data channel
   * @param {string} role - 'sender' or 'recipient'
   * @returns {boolean} - true if channel was stored, false if it was rejected
   */
  function storeIncomingChannel(contactGuid, channel, role) {
    if (!window._telecomDataChannels) {
      window._telecomDataChannels = new Map();
    }
    
    const metadata = channelMetadata.get(contactGuid) || { role };
    const existingOutgoing = metadata.outgoing;
    
    // 🚨 CRITICAL RULE: On recipient side, NEVER overwrite outgoing channel with incoming channel
    if (role === 'recipient' && existingOutgoing) {
      console.log('[ChannelManager] 🚨 BLOCKED: Incoming channel would overwrite outgoing channel on recipient side!');
      console.log('[ChannelManager] Outgoing channel preserved:', {
        contactGuid,
        outgoingChannelId: existingOutgoing.id,
        outgoingReadyState: existingOutgoing.readyState,
        incomingChannelId: channel.id,
        incomingReadyState: channel.readyState
      });
      
      // Store incoming channel separately for receiving, but don't overwrite outgoing
      metadata.incoming = channel;
      channelMetadata.set(contactGuid, metadata);
      
      // DO NOT update main map - keep outgoing channel there for sending
      return false; // Channel not stored in main map
    }
    ...
    // Fallback: store incoming if no outgoing exists
    metadata.incoming = channel;
    channelMetadata.set(contactGuid, metadata);
    window._telecomDataChannels.set(contactGuid, channel);
    
    console.log(`[ChannelManager] ✅ Stored INCOMING channel for ${role} (no outgoing exists):`, {
      contactGuid,
      channelId: channel.id,
      label: channel.label,
      readyState: channel.readyState
  ...
  /**
   * Get channel for sending messages (always returns outgoing channel)
   * @param {string} contactGuid - Contact GUID
   * @returns {RTCDataChannel|null} - Outgoing channel for sending, or null if not found
   */
  function getChannelForSending(contactGuid) {
    const metadata = channelMetadata.get(contactGuid);
    if (!metadata) {
      // Fallback to main map (backward compatibility)
      return window._telecomDataChannels?.get(contactGuid) || null;
    }
    
    // Always use outgoing channel for sending
    const outgoingChannel = metadata.outgoing;
    if (outgoingChannel && outgoingChannel.readyState === 'open') {
      return outgoingChannel;
    }
    
    // If outgoing not open, check if we can use it anyway (might open soon)
    if (outgoingChannel) {
      return outgoingChannel;
    }
    
    // Fallback: if no outgoing, use main map (for backward compatibility)
    return window._telecomDataChannels?.get(contactGuid) || null;
  }
  
  /**
   * Get channel for receiving messages (prefers incoming, falls back to outgoing)
   * @param {string} contactGuid - Contact GUID
   * @returns {RTCDataChannel|null} - Channel for receiving, or null if not found
   */
  function getChannelForReceiving(contactGuid) {
    const metadata = channelMetadata.get(contactGuid);
    if (!metadata) {
      // Fallback to main map (backward compatibility)
      return window._telecomDataChannels?.get(contactGuid) || null;
    ...
    // Prefer incoming channel for receiving
    if (metadata.incoming) {
      return metadata.incoming;
    }
    
    // Fallback to outgoing if no incoming
    return metadata.outgoing || null;
  }
   ...
   * @param {string} contactGuid - Contact GUID
   */
  function removeChannel(contactGuid) {
    channelMetadata.delete(contactGuid);
  }
  
  /**
   * Get channel metadata for debugging
   * @param {string} contactGuid - Contact GUID
   * @returns {Object|null} - Channel metadata
   */
  function getMetadata(contactGuid) {
    return channelMetadata.get(contactGuid) || null;
  }
  
  return {
    storeOutgoingChannel,
    storeIncomingChannel,
    getChannelForSending,
    getChannelForReceiving,
    removeChannel,
    getMetadata
  };
})();
...
function getConnectionStateForContact(contactGuid) {
  ...
  // If connectionState is 'connected', we're connected
  if (pc.connectionState === 'connected') {
    return 'connected';
  }
  
  // If connectionState is 'connecting', we're connecting
  if (pc.connectionState === 'connecting') {
    return 'connecting';
  }
  
  // If connectionState is 'disconnected', 'failed', or 'closed', we're disconnected
  if (pc.connectionState === 'disconnected' || pc.connectionState === 'failed' || pc.connectionState === 'closed') {
    return 'disconnected';
  }
  
  // If PC exists but connectionState is 'new' or undefined, and data channel is not open,
  // consider it disconnected (connection was lost or never established)
  // This covers the case after page refresh when PC exists but connection is broken
  return 'disconnected';
}
...
/**
 * Setup messages channel handler - UNIFIED FUNCTION for all data channel message handling
 * This replaces duplicate handler setup logic scattered across 8+ places in the codebase
 * 
 * @param {RTCDataChannel} channel - The data channel to set up handler for
 * @param {string} contactGuid - The contact GUID
 * @param {string} role - 'sender' or 'recipient' (for logging purposes)
 * @param {Object} options - Optional configuration
 * @param {boolean} options.overwrite - If true, always overwrite existing handler (default: true)
 * @param {Function} options.onOpenCallback - Optional callback when channel opens
 */
function setupMessagesChannelHandler(channel, contactGuid, role = 'unknown', options = {}) {
  if (!channel || !contactGuid) {
    console.warn('[Telecom] setupMessagesChannelHandler: Invalid parameters', { channel: !!channel, contactGuid });
    return;
  }

  const { overwrite = true, onOpenCallback = null } = options;
  
  // Create the message handler function (reusable logic)
  const createMessageHandler = () => {
    return async (event) => {
      try {
        ...
        if (messageData.type === 'message' && messageData.text) {
          ...
          if (messageData.encrypted) {
            try {
              ...
              console.error(`[Telecom] [${role.toUpperCase()}] Error decrypting:`, e);
              decryptedText = '[Encrypted message - decryption failed]';
            }
          }
        ...
        console.error(`[Telecom] [${role.toUpperCase()}] Error parsing message:`, e);
      }
    };
  };

  // 🚨 CRITICAL: Handle race condition - channel might already be open
  // ALWAYS set handler immediately if channel is open, regardless of overwrite flag
  // This ensures messages are received even if handler was set incorrectly before
  if (channel.readyState === 'open') {
    console.log(`[Telecom] [${role.toUpperCase()}] ⚠️ Channel already open - setting up handler IMMEDIATELY`);
    console.log(`[Telecom] [${role.toUpperCase()}] Current handler exists:`, !!channel.onmessage);
    // ALWAYS overwrite if channel is open - this is critical for receiving messages
    channel.onmessage = createMessageHandler();
    console.log(`[Telecom] [${role.toUpperCase()}] ✅ Handler set up for already-open channel (overwritten if existed)`);
  }
  ...
  channel.onopen = () => {
    ...
    // Set up handler if missing or overwrite is true
    if (overwrite || !channel.onmessage) {
      channel.onmessage = createMessageHandler();
      console.log(`[Telecom] [${role.toUpperCase()}] ✅ Handler set up in onopen callback`);
    } else {
    ...
    if (onOpenCallback) {
      try {
        onOpenCallback(channel);
      } catch (e) {
        console.error(`[Telecom] [${role.toUpperCase()}] Error in onOpenCallback:`, e);
      }
    }
  };
...
function setupControlChannelHandler(controlChannel, contactGuid, pc, config, storageKey) {
  ...
  controlChannel.onmessage = async (event) => {
    try {
      ...
      // Handle one-tap reconnect offer (URL link sent via control channel)
      if (messageData.type === 'onetap-reconnect-offer' && messageData.offerUrl) {
        console.log('[Telecom] 🔄 Received one-tap reconnect offer URL from contact:', contactGuid);
        
        try {
          // Set hash to trigger one-tap processing
          const hash = new URL(messageData.offerUrl).hash;
          if (hash && hash.startsWith('#offer=')) {
            // Temporarily set location.hash to trigger one-tap processing
            const originalHash = location.hash;
            location.hash = hash;
            
            // Process will happen via bootstrapFromHash in launch function
            console.log('[Telecom] ✅ One-tap reconnect offer hash set, processing...');
            
            // Restore original hash after a delay (if needed)
            setTimeout(() => {
            ...
            console.warn('[Telecom] Invalid one-tap reconnect offer URL format');
          }
        } catch (e) {
          console.error('[Telecom] ❌ Error processing one-tap reconnect offer:', e);
        }
        return;
      }
    } catch (e) {
...
async function autoReconnectToContact(contactGuid, config, storageKey) {
  ...
  try {
    // CRITICAL: Close existing PC before creating new one to avoid conflicts
    const existingPC = window._telecomPeerConnections?.get(contactGuid);
    if (existingPC) {
      console.log('[Telecom] ⚠️ Closing existing PC before auto-reconnect');
      try {
        existingPC.close();
      } catch (e) {
        console.warn('[Telecom] Error closing existing PC:', e);
      }
    ...
    // CRITICAL: Check if control channel exists and is open before attempting to send
    const waitForControlChannel = () => {
      return new Promise((resolve) => {
        if (!controlChannel) {
          console.warn('[Telecom] ⚠️ No control channel available for auto-reconnect');
          resolve(false); // Return false if no channel
          return;
        }
        ...
        if (controlChannel.readyState === 'open') {
          ...
          setTimeout(() => {
            clearInterval(checkInterval);
            console.warn('[Telecom] ⚠️ Control channel timeout - channel not ready after 5 seconds');
            resolve(false);
          }, 5000);
        }
      });
    ...
    const channelReady = await waitForControlChannel();
    
    if (channelReady && controlChannel && controlChannel.readyState === 'open') {
      controlChannel.send(JSON.stringify({
      ...
      console.warn('[Telecom] ⚠️ Control channel not ready for auto-reconnect');
      console.warn('[Telecom] 💡 Auto-reconnect requires control channel - use manual reconnect button instead');
      // Set up handler for when channel opens (if it exists)
      if (controlChannel) {
        controlChannel.onopen = () => {
          console.log('[Telecom] Control channel opened - sending reconnect offer');
          controlChannel.send(JSON.stringify({
            type: 'reconnect-offer',
      ...
      // Return false to indicate auto-reconnect didn't work
      return false;
    }
...
/**
 * Send reconnect link as a message in chat
 */
async function sendReconnectLinkAsMessage(contactGuid, linkUrl, win, winId, config, storageKey) {
  console.log('[Telecom] Sending reconnect link as message to contact:', contactGuid);
  
  const chatId = `contact-${contactGuid}`;
  const effectiveGuid = getEffectiveGuid(config);
  
  // Create message object with link
  const messageText = `🔄 Reconnect link: ${linkUrl}`;
  const newMessage = {
    id: 'msg-' + Date.now(),
    chatId: chatId,
    ...
    timestamp: new Date().toISOString(),
    type: 'user',
    isReconnectLink: true,
    reconnectUrl: linkUrl
  };
  ...
  if (dataChannel && dataChannel.readyState === 'open') {
    try {
      ...
      if (recipientPublicKey) {
        try {
          encryptedText = await encryptMessageForTelecom(messageText, recipientPublicKey);
        } catch (e) {
          console.warn('[Telecom] ⚠️ Sending unencrypted reconnect link due to encryption error');
        }
      }
      ...
      console.log('[Telecom] ✅ Reconnect link sent via WebRTC to contact:', contactGuid);
    } catch (e) {
      console.error('[Telecom] Error sending reconnect link via WebRTC:', e);
    }
  }
  ...
  // Refresh messages display if this chat is selected
  const selectedChatId = win.dataset.selectedChatId;
  if (selectedChatId === chatId && chat) {
    selectChat(win, winId, chat, config, storageKey);
  }
  
  console.log('[Telecom] ✅ Reconnect link saved as message in chat');
}

/**
 * Handle reconnect button click - create new one-tap offer link
 */
async function handleReconnectClick(contactGuid, win, winId, config, storageKey) {
  console.log('[Telecom] 🔄 Reconnect button clicked for contact:', contactGuid);
  
  if (!contactGuid) {
    console.warn('[Telecom] ⚠️ No contactGuid provided for reconnect');
    return;
  }
  
  // CRITICAL: Check if connection is already active - no need to reconnect
  const existingPC = window._telecomPeerConnections?.get(contactGuid);
  if (existingPC) {
    const connectionState = existingPC.connectionState;
    const iceConnectionState = existingPC.iceConnectionState;
    console.log('[Telecom] 📊 Current connection state:', {
      connectionState,
      iceConnectionState,
      signalingState: existingPC.signalingState
    });
    
    // If connection is already established, no need to reconnect
    if (connectionState === 'connected' && iceConnectionState === 'connected') {
      console.log('[Telecom] ✅ Connection already active - no reconnect needed');
      if (window.Dialog && window.Dialog.alert) {
        await window.Dialog.alert('Connection is already established. No reconnect needed.');
      } else {
        alert('Connection is already established. No reconnect needed.');
      }
      return;
    }
    
    // If connection is in progress, warn user
    if (connectionState === 'connecting' || iceConnectionState === 'checking') {
      console.log('[Telecom] ⏳ Connection already in progress - reconnect may conflict');
      if (window.Dialog && window.Dialog.confirm) {
        const confirmed = await window.Dialog.confirm('Connection is already in progress. Do you want to reconnect anyway?');
        if (!confirmed) {
          return;
        }
      } else if (!confirm('Connection is already in progress. Do you want to reconnect anyway?')) {
        return;
      }
    }
  }
  
  // Check if OneTapTelecom is available
  console.log('[Telecom] 🔍 Checking OneTapTelecom availability...', {
    OneTapTelecomExists: !!window.OneTapTelecom,
    hasCreateOfferLink: !!(window.OneTapTelecom && typeof window.OneTapTelecom.createOfferLink === 'function')
  });
  
  if (!window.OneTapTelecom || typeof window.OneTapTelecom.createOfferLink !== 'function') {
    console.error('[Telecom] ❌ OneTapTelecom.createOfferLink not available');
    if (window.Dialog && window.Dialog.alert) {
      await window.Dialog.alert('One-tap reconnect is not available. Please use the manual invite system.');
    } else {
      alert('One-tap reconnect is not available. Please use the manual invite system.');
    }
    return;
  }
  ...
  try {
    ...
    const reconnectBtn = win.querySelector(`.telecom-reconnect-btn[data-contact-guid="${contactGuid}"]`);
    console.log('[Telecom] 🔘 Reconnect button element:', reconnectBtn ? 'found' : 'not found');
    if (reconnectBtn) {
      reconnectBtn.textContent = '⏳';
      reconnectBtn.style.cursor = 'wait';
      reconnectBtn.disabled = true;
      console.log('[Telecom] 🔘 Button state updated to loading');
    }
    
    // Create new offer link via one-tap
    console.log('[Telecom] 🔗 Creating one-tap offer link for reconnect...', {
      contactGuid: contactGuid,
      hasConfig: !!config
    });
    const link = await window.OneTapTelecom.createOfferLink(contactGuid, config);
    console.log('[Telecom] 📊 createOfferLink result:', {
      hasLink: !!link,
      hasUrl: !!(link && link.url),
      urlPreview: link && link.url ? link.url.substring(0, 100) + '...' : null,
      tokenBytes: link ? link.tokenBytes : null,
      sdpBytes: link ? link.sdpBytes : null
    });
    
    if (!link || !link.url) {
      console.error('[Telecom] ❌ Failed to create offer link:', link);
      throw new Error('Failed to create offer link');
    }
    
    console.log('[Telecom] ✅ Offer link created for reconnect:', link.url.substring(0, 100) + '...');
    
    // Check peer connection and control channel status
    const pc = window._telecomPeerConnections?.get(contactGuid);
    const controlChannel = window._telecomControlChannels?.get(contactGuid);
    console.log('[Telecom] 📡 Connection status:', {
      hasPeerConnection: !!pc,
      peerConnectionState: pc ? pc.connectionState : null,
      hasControlChannel: !!controlChannel,
      controlChannelState: controlChannel ? controlChannel.readyState : null
    });
    
    // Always show popup with reconnect link (user will copy and send it manually)
    console.log('[Telecom] 📤 Showing one-tap link dialog...');
    showOneTapLinkDialog(link.url, link.tokenBytes, link.sdpBytes, winId);
    console.log('[Telecom] ✅ Dialog shown, user can now copy link and send it in chat');
    ...
    if (reconnectBtn) {
      ...
      console.log('[Telecom] 🔘 Button state restored');
    }
    
    // Update connection status to connecting
    console.log('[Telecom] 🔄 Updating connection status to "connecting"...');
    updateConnectionStatusForChat(contactGuid, 'connecting');
    
  } catch (error) {
    console.error('[Telecom] ❌ Error during reconnect:', error);
    console.error('[Telecom] Error stack:', error.stack);
    ...
    if (reconnectBtn) {
      ...
      reconnectBtn.style.cursor = 'pointer';
      reconnectBtn.disabled = false;
      console.log('[Telecom] 🔘 Button state restored after error');
    }
    
    // Show error message
    const errorMsg = error.message || 'Failed to reconnect. Please try again.';
    console.log('[Telecom] 📢 Showing error message to user:', errorMsg);
    if (window.Dialog && window.Dialog.alert) {
...
function renderChatsList(win, winId, config, storageKey) {
        ...
        <div style="flex:1; min-width:0;">
          <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:4px;">
            ...
            <div style="display:flex; align-items:center; gap:6px; flex-shrink:0;">
              ${showReconnectButton ? `<button class="telecom-reconnect-btn" data-contact-guid="${chat.contactGuid}" style="background:none; border:none; font-size:16px; cursor:pointer; color:var(--accent); padding:4px; min-width:24px; height:24px; display:flex; align-items:center; justify-content:center; opacity:0.8; transition:all 0.2s; flex-shrink:0;" title="${I18n.t('telecom.reconnect') || 'Reconnect'}" onmouseover="this.style.opacity='1'; this.style.transform='scale(1.1)';" onmouseout="this.style.opacity='0.8'; this.style.transform='scale(1)';">🔄</button>` : ''}
              ${lastMessageTime ? `<div style="font-size:12px; color:var(--muted); margin-left:${showReconnectButton ? '0' : '8px'};">${lastMessageTime}</div>` : ''}
            </div>
          </div>
          ${lastMessageText ? `
  ...
  chatItems.forEach(item => {
    ...
    item.addEventListener('click', (e) => {
      // Don't trigger chat selection if clicking on reconnect button
      if (e.target.closest('.telecom-reconnect-btn')) {
        e.stopPropagation();
        return;
      }
    ...
    const reconnectBtn = item.querySelector('.telecom-reconnect-btn');
    if (reconnectBtn) {
      reconnectBtn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const contactGuid = reconnectBtn.dataset.contactGuid;
        if (contactGuid) {
          await handleReconnectClick(contactGuid, win, winId, config, storageKey);
        }
      });
    }
...
function selectChat(win, winId, chat, config, storageKey) {
  // NOTE: Auto-reconnect через control channel отключен - используем только one-tap механизм
  // Auto-reconnect через control channel не работает после refresh страницы (control channel закрыт)
  // Пользователь должен использовать кнопку Reconnect для генерации one-tap ссылки
  // 
  // Старый код (отключен):
  // if (chat.type === 'contact' && chat.contactGuid) {
  //   const connectionState = getConnectionStateForContact(chat.contactGuid);
...
async function sendContactInvite(targetGuid, senderAccount, senderEffectiveGuid) {
  ...
  if (typeof RTCPeerConnection !== 'undefined') {
    try {
      ...
      // 🚨 Use ChannelManager to store outgoing channel (sender side)
      if (window.TelecomChannelManager) {
        window.TelecomChannelManager.storeOutgoingChannel(targetGuid, dataChannel, 'sender');
      } else {
        window._telecomDataChannels.set(targetGuid, dataChannel);
      }
      
      // Handle incoming data channels from recipient (for trickle ICE)
      console.log('[Telecom] 🚨 CRITICAL: Setting up ondatachannel handler in sendContactInvite for contact:', targetGuid);
      pc.ondatachannel = (event) => {
        const incomingChannel = event.channel;
        console.log('[Telecom] 📥 Incoming data channel event (sender):', {
          label: incomingChannel.label,
          contactGuid: targetGuid,
          readyState: incomingChannel.readyState,
          channelId: incomingChannel.id,
          timestamp: new Date().toISOString()
        });
        console.log('[Telecom] 🚨 CRITICAL: ondatachannel FIRED on sender side!');
        
        // Handle control channel
        if (incomingChannel.label === 'control') {
          console.log('[Telecom] ✅ Incoming control channel (sender)');
          if (!window._telecomControlChannels) {
            window._telecomControlChannels = new Map();
          }
        ...
        // 🚨 Use ChannelManager to store incoming channel (sender side - replaces outgoing)
        // CRITICAL: On sender side, incoming channel REPLACES outgoing channel (this is correct)
        const existingChannel = window._telecomDataChannels?.get(targetGuid);
        const existingOnMessage = existingChannel?.onmessage;
        console.log('[Telecom] 📥 Incoming messages channel (sender):', {
          contactGuid: targetGuid,
          replacingExisting: !!existingChannel,
          existingChannelLabel: existingChannel?.label,
          existingChannelReadyState: existingChannel?.readyState,
          newChannelReadyState: incomingChannel.readyState,
          hasExistingOnMessage: !!existingOnMessage
        });
        
        if (window.TelecomChannelManager) {
          window.TelecomChannelManager.storeIncomingChannel(targetGuid, incomingChannel, 'sender');
        } else {
          window._telecomDataChannels.set(targetGuid, incomingChannel);
        }
        console.log('[Telecom] ✅ Stored incoming data channel (messages) for contact:', targetGuid);
        
        // CRITICAL: If there was an existing onmessage handler, we need to make sure it's not lost
        // But actually, the incoming channel should have its own handler set up below
        // The issue is that processWebRTCAnswer might overwrite it later
        
        // Set up message handler for trickle ICE candidates, answers, AND regular messages
        console.log('[Telecom] 🚨 CRITICAL: Setting up onmessage handler for incoming channel in sendContactInvite');
        console.log('[Telecom] Channel state:', {
          readyState: incomingChannel.readyState,
          label: incomingChannel.label,
          id: incomingChannel.id,
          hasExistingHandler: !!incomingChannel.onmessage
        });
        
        incomingChannel.onmessage = async (event) => {
          try {
            console.log('[Telecom] 📥 Message received via incoming data channel (sender side):', {
              contactGuid: targetGuid,
              dataLength: event.data ? event.data.length : 0,
              dataPreview: event.data ? (event.data.substring(0, 100) + (event.data.length > 100 ? '...' : '')) : 'no data',
              timestamp: new Date().toISOString()
            });
            console.log('[Telecom] 🚨 CRITICAL: Handler FIRED in sendContactInvite!');
            
            const messageData = JSON.parse(event.data);
            console.log('[Telecom] 📥 Parsed message from incoming channel:', {
              type: messageData.type,
              hasText: !!messageData.text,
              encrypted: messageData.encrypted || false,
              hasWebrtcAnswer: !!messageData.webrtcAnswer
            });
              ...
              try {
                ...
                if (sentInvitesData) {
                  ...
                  if (invite) {
                    ...
                    console.log('[Telecom] 🚨 CRITICAL: About to call processWebRTCAnswer from sendContactInvite handler');
                    console.log('[Telecom] Invite details:', {
                      inviteId: invite.id,
                      toGuid: invite.toGuid,
            ...
            if (messageData.type === 'message' && messageData.text) {
              console.log('[Telecom] ✅ Processing regular message from incoming channel (sender side):', {
                contactGuid: targetGuid,
                messageLength: messageData.text.length,
                encrypted: messageData.encrypted || false
              });
              
              // Use the same message handling logic as in processWebRTCAnswer
              const chatId = `contact-${targetGuid}`;
              ...
              const newMessage = {
                ...
                chatId: chatId,
                senderId: targetGuid,
                senderName: messageData.senderName || targetGuid.substring(0, 8) + '...',
                text: decryptedText,
                timestamp: messageData.timestamp || new Date().toISOString(),
                type: 'user',
                viaWebRTC: true,
              ...
              messages.push(newMessage);
              localStorage.setItem(MESSAGES_STORAGE_KEY, JSON.stringify(messages));
              console.log('[Telecom] ✅ Message saved from incoming channel (sender side)');
              
              // Update chat and refresh UI
              const chats = getChats();
              const chat = chats.find(c => c.id === chatId);
              if (chat) {
                chat.lastMessage = {
                ...
                telecomWindows.forEach(winEl => {
                  ...
                  if (winId) {
                    ...
                    if (win) {
                      ...
                      // Refresh messages if this chat is selected
                      const selectedChatId = win.dataset.selectedChatId;
                      if (selectedChatId === chatId) {
                        selectChat(win, winId, chat, effectiveConfig, effectiveStorageKey);
                      }
        ...
        incomingChannel.onopen = () => {
          ...
          console.log('[Telecom] 📡 Incoming channel state:', {
            contactGuid: targetGuid,
            readyState: incomingChannel.readyState,
            label: incomingChannel.label,
            hasOnMessageHandler: !!incomingChannel.onmessage
          });
          updateConnectionStatusForChat(targetGuid, 'connected');
          
          // Verify that message handler is set up
          if (!incomingChannel.onmessage) {
            console.error('[Telecom] ❌ CRITICAL: Incoming channel opened but onmessage handler is not set!');
            console.error('[Telecom] This means messages from recipient will not be received!');
          } else {
            console.log('[Telecom] ✅ Incoming channel has onmessage handler set up');
          }
        };
        
        // 🚨 CRITICAL: Also set up handler immediately if channel is already open
        // If channel is already open, onopen won't fire, so we need to verify handler is set NOW
        if (incomingChannel.readyState === 'open') {
          console.log('[Telecom] ⚠️ Incoming channel already open, verifying handler...');
          if (!incomingChannel.onmessage) {
            console.error('[Telecom] ❌ CRITICAL: Incoming channel is already open but has no onmessage handler! Setting up NOW...');
            // Handler was set above, but if channel is already open, we need to verify it's there
            // The handler above should have been set, but let's double-check
            console.log('[Telecom] ⚠️ Handler should have been set above - this is a race condition!');
          } else {
            console.log('[Telecom] ✅ Incoming channel already open and has onmessage handler');
            console.log('[Telecom] Handler function:', typeof incomingChannel.onmessage);
          }
        }
      };
...
async function processIncomingSignaling(effectiveGuid, config) {
  ...
  try {
    ...
    if (!sentInvitesData) {
      // No sent invites, remove all answers
    ...
    for (const peerId in signaling) {
      if (signaling[peerId] && signaling[peerId].answers) {
        const originalLength = signaling[peerId].answers.length;
        signaling[peerId].answers = signaling[peerId].answers.filter(answer => {
          return validInviteIds.has(answer.inviteId);
        });
        
        if (signaling[peerId].answers.length !== originalLength) {
          hasChanges = true;
        }
        
        // Remove peer entry if no answers left
        if (signaling[peerId].answers.length === 0) {
          delete signaling[peerId];
        }
      }
    }
    ...
    console.error('[Telecom] Error in processIncomingSignaling cleanup:', e);
  }
}

// Export processIncomingSignaling for testing - execute immediately after function definition
// Make it available both as window.Telecom.processIncomingSignaling and as a global function
// Use immediate execution to ensure export happens right after function definition
(function() {
  // Try to export to window if available (browser environment)
  if (typeof window !== 'undefined') {
    if (!window.Telecom) {
      window.Telecom = {};
    }
    window.Telecom.processIncomingSignaling = processIncomingSignaling;
    window.processIncomingSignaling = processIncomingSignaling;
  }
  
  // Also make it available in global scope for Node.js/JSDOM testing
  if (typeof global !== 'undefined') {
    if (!global.Telecom) {
      global.Telecom = {};
    }
    global.Telecom.processIncomingSignaling = processIncomingSignaling;
    global.processIncomingSignaling = processIncomingSignaling;
  }
  
  // Make it available as a global variable (for eval contexts)
  if (typeof globalThis !== 'undefined') {
    if (!globalThis.Telecom) {
      globalThis.Telecom = {};
    }
    globalThis.Telecom.processIncomingSignaling = processIncomingSignaling;
    globalThis.processIncomingSignaling = processIncomingSignaling;
  }
  
  // Also assign to global scope directly (for eval contexts)
  // This works in non-strict mode eval contexts
  try {
    // Use Function constructor to access global scope
    const setGlobal = new Function('fn', `
      if (typeof window !== 'undefined') {
        if (!window.Telecom) window.Telecom = {};
        window.Telecom.processIncomingSignaling = fn;
        window.processIncomingSignaling = fn;
      }
      if (typeof global !== 'undefined') {
        if (!global.Telecom) global.Telecom = {};
        global.Telecom.processIncomingSignaling = fn;
        global.processIncomingSignaling = fn;
      }
      if (typeof globalThis !== 'undefined') {
        if (!globalThis.Telecom) globalThis.Telecom = {};
        globalThis.Telecom.processIncomingSignaling = fn;
        globalThis.processIncomingSignaling = fn;
      }
    `);
    setGlobal(processIncomingSignaling);
  } catch (e) {
    // Ignore errors
  }
})();
  ...
  if (response === 'accepted') {
    ...
    if (hasValidOffer && typeof RTCPeerConnection !== 'undefined') {
      try {
        ...
        // 🚨 Use ChannelManager to store outgoing channel (recipient side)
        if (window.TelecomChannelManager) {
          window.TelecomChannelManager.storeOutgoingChannel(inviteForAnswer.fromGuid, dataChannel, 'recipient');
        } else {
        ...
        pc.ondatachannel = (event) => {
          ...
          // 🚨 Use ChannelManager to store incoming channel (will prevent overwriting outgoing on recipient side)
          if (window.TelecomChannelManager) {
            const stored = window.TelecomChannelManager.storeIncomingChannel(inviteForAnswer.fromGuid, incomingChannel, 'recipient');
            if (!stored) {
              // Channel was not stored in main map (outgoing preserved) - set up handler and return
              setupMessagesChannelHandler(incomingChannel, inviteForAnswer.fromGuid, 'recipient', { overwrite: true });
              return;
            }
          } else {
            // Fallback: old logic
            const existingOutgoingChannel = window._telecomDataChannels?.get(inviteForAnswer.fromGuid);
            if (existingOutgoingChannel) {
              console.log('[Telecom] ⚠️ Incoming channel received on recipient side, but outgoing channel exists - NOT overwriting');
              setupMessagesChannelHandler(incomingChannel, inviteForAnswer.fromGuid, 'recipient', { overwrite: true });
              return;
            }
        ...
        // For trickle ICE: start with empty candidates, they'll be added incrementally
        // But include any candidates that were already collected during offer processing
        const currentCandidates = iceCandidates.length > 0 ? [...iceCandidates] : [];
        const webrtcAnswer = {
          sdp: answer.sdp,
          type: answer.type,
          trickle: true, // Flag indicating trickle ICE
          iceCandidates: currentCandidates // Include any candidates already collected
        };
        
        console.log('[TRICKLE-RECIPIENT] 📊 Initial answer created:', {
          sdpLength: webrtcAnswer.sdp.length,
          type: webrtcAnswer.type,
          trickle: webrtcAnswer.trickle,
          initialCandidatesCount: currentCandidates.length
        });
        ...
        // Save answer to invite in recipient storage immediately
        ...
        // Always use current webrtcAnswer (which includes all collected candidates so far)
        const sendAnswerThroughChannel = (channel, channelName, answerToSend = webrtcAnswer) => {
        ...
        // Use current webrtcAnswer which includes any candidates already collected
        ...
        console.log('[TRICKLE-RECIPIENT] 📊 Answer to send:', {
          type: webrtcAnswer.type,
          sdpLength: webrtcAnswer.sdp.length,
          candidatesCount: webrtcAnswer.iceCandidates.length,
          trickle: webrtcAnswer.trickle
        });
        ...
        if (dataChannel && dataChannel.readyState === 'open') {
          ...
          const sendStartTime = Date.now();
          answerSent = sendAnswerThroughChannel(dataChannel, 'outgoing data channel (immediate)', webrtcAnswer);
          const sendTime = Date.now() - sendStartTime;
        ...
        if (incomingChannel && incomingChannel.readyState === 'open' && !answerSent) {
          ...
          const sendStartTime = Date.now();
          answerSent = sendAnswerThroughChannel(incomingChannel, 'incoming data channel (immediate)', webrtcAnswer);
          const sendTime = Date.now() - sendStartTime;
          console.log(`[TRICKLE-RECIPIENT] ⏱️ Answer send attempt completed in ${sendTime}ms, success: ${answerSent}`);
        } else if (!answerSent) {
        ...
        if (!answerSent) {
          ...
          try {
            ...
            if (effectiveGuid) {
              ...
              console.log('[TRICKLE-RECIPIENT] 📝 Saving answer to localStorage:', {
                ...
                inviteId: invite.id,
                fromGuid: invite.fromGuid,
                answerType: webrtcAnswer.type,
                answerSdpLength: webrtcAnswer.sdp.length
              });
          ...
          // Wait a few seconds for ICE candidates to be collected before showing dialog
          // This ensures that manually copied answer includes candidates for connection establishment
          console.log('[TRICKLE-RECIPIENT] ⏳ Waiting for ICE candidates before showing dialog (for manual copy)...');
          const candidateWaitStartTime = Date.now();
          const candidateWaitTimeout = 5000; // Wait up to 5 seconds for candidates
          const minCandidatesWait = 2000; // Minimum wait of 2 seconds
          const targetRelayCandidates = 1; // Try to get at least 1 relay candidate
          
          // Wait for candidates with timeout
          const waitForCandidates = new Promise((resolve) => {
            ...
            const checkCandidates = () => {
              const currentCandidates = webrtcAnswer.iceCandidates.length;
              const relayCandidates = webrtcAnswer.iceCandidates.filter(c => 
                c.candidate && c.candidate.includes('typ relay')
              ).length;
              
              console.log(`[TRICKLE-RECIPIENT] 📊 Candidate check: ${currentCandidates} total, ${relayCandidates} relay, gathering state: ${pc.iceGatheringState}`);
              
              // If we have enough candidates (especially relay) and minimum wait passed, resolve
              if (!resolved && (relayCandidates >= targetRelayCandidates || pc.iceGatheringState === 'complete')) {
                const waitTime = Date.now() - candidateWaitStartTime;
                if (waitTime >= minCandidatesWait) {
                  resolved = true;
                  if (checkInterval) clearInterval(checkInterval);
                  if (timeoutId) clearTimeout(timeoutId);
                  if (minWaitId) clearTimeout(minWaitId);
                  console.log(`[TRICKLE-RECIPIENT] ✅ Candidate wait complete: ${currentCandidates} candidates (${relayCandidates} relay) after ${waitTime}ms`);
                  resolve();
                }
              }
            };
            ...
            // Check every 200ms
            checkInterval = setInterval(checkCandidates, 200);
            ...
            timeoutId = setTimeout(() => {
              if (!resolved) {
                ...
                const finalCandidates = webrtcAnswer.iceCandidates.length;
                const finalRelay = webrtcAnswer.iceCandidates.filter(c => 
                  c.candidate && c.candidate.includes('typ relay')
                ).length;
                console.log(`[TRICKLE-RECIPIENT] ⏱️ Candidate wait timeout: ${finalCandidates} candidates (${finalRelay} relay) after ${candidateWaitTimeout}ms`);
                resolve();
              }
            }, candidateWaitTimeout);
            
            // Minimum wait
            minWaitId = setTimeout(() => {
              checkCandidates(); // Check immediately after minimum wait
            }, minCandidatesWait);
          });
          
          // Wait for candidates, then show dialog
          waitForCandidates.then(() => {
            // Update inviteWithAnswer with latest candidates before showing dialog
            inviteWithAnswer.webrtcAnswer = webrtcAnswer;
            
            // Update localStorage with latest answer (including all collected candidates)
            try {
              const effectiveGuid = getEffectiveGuid(config);
              if (effectiveGuid) {
                const SENT_INVITES_STORAGE_KEY = `webos.telecom.sent_invites.guid_from.${invite.fromGuid}`;
                ...
                if (sentInvitesData) {
                  ...
                  if (inviteIndex !== -1) {
                    sentInvites[inviteIndex] = inviteWithAnswer;
                    localStorage.setItem(SENT_INVITES_STORAGE_KEY, JSON.stringify(sentInvites));
                    console.log(`[TRICKLE-RECIPIENT] ✅ Updated answer in localStorage with ${webrtcAnswer.iceCandidates.length} candidate(s) before showing dialog`);
                  }
                }
              }
            } catch (e) {
              console.error('[TRICKLE-RECIPIENT] Error updating answer in localStorage:', e);
            }
            
            // Show dialog with answer that includes collected candidates
            console.log('[TRICKLE-RECIPIENT] 📋 Showing manual share dialog with', webrtcAnswer.iceCandidates.length, 'candidate(s)');
            ...
            console.error('[TRICKLE-RECIPIENT] ❌ Error waiting for candidates:', e);
            // Show dialog anyway (with whatever candidates we have)
            console.log('[TRICKLE-RECIPIENT] 📋 Showing dialog anyway with', webrtcAnswer.iceCandidates.length, 'candidate(s)');
            try {
              showShareAnswerDialog(winId || null, inviteWithAnswer, config, storageKey);
            } catch (err) {
              console.error('[TRICKLE-RECIPIENT] ❌ Error showing manual share dialog:', err);
            }
          });
        } else {
        ...
        const checkAndSendAnswer = () => {
          ...
          if (dataChannel && dataChannel.readyState === 'open' && !answerSent) {
            answerSent = sendAnswerThroughChannel(dataChannel, 'outgoing data channel (periodic check)', webrtcAnswer);
            if (answerSent) {
              clearInterval(checkInterval);
              checkInterval = null;
              return;
          ...
          if (incomingChannelCheck && incomingChannelCheck.readyState === 'open' && !answerSent) {
            answerSent = sendAnswerThroughChannel(incomingChannelCheck, 'incoming data channel (periodic check)', webrtcAnswer);
            if (answerSent) {
              clearInterval(checkInterval);
              checkInterval = null;
              return;
        ...
        pc.onicecandidate = (event) => {
          ...
          if (event.candidate) {
            ...
            // Update webrtcAnswer with new candidate (for manual copying and data channel sending)
            webrtcAnswer.iceCandidates.push(candidate);
            
            // Update inviteWithAnswer with updated answer (for manual dialog)
            inviteWithAnswer.webrtcAnswer = webrtcAnswer;
            
            // Update recipient storage with updated answer
            recipientInvites[recipientInviteIndex].webrtcAnswer = webrtcAnswer;
            ...
            // Update localStorage with updated answer (including new candidate) for sender polling
            try {
              const effectiveGuid = getEffectiveGuid(config);
              if (effectiveGuid) {
                const SENT_INVITES_STORAGE_KEY = `webos.telecom.sent_invites.guid_from.${invite.fromGuid}`;
                ...
                if (sentInvitesData) {
                  ...
                  const inviteIndex = sentInvites.findIndex(inv => inv.id === invite.id);
                  if (inviteIndex !== -1) {
                    // Update entire answer with all candidates collected so far
                    sentInvites[inviteIndex].webrtcAnswer = webrtcAnswer;
                    localStorage.setItem(SENT_INVITES_STORAGE_KEY, JSON.stringify(sentInvites));
                    console.log(`[TRICKLE-RECIPIENT] ✅ Answer updated in localStorage with ${webrtcAnswer.iceCandidates.length} candidate(s)`);
                  }
                }
              }
            } catch (e) {
              console.error('[TRICKLE-RECIPIENT] Error saving candidate to localStorage:', e);
            }
            
            // Update dialog if it's open (refresh answer data)
            try {
              const dialog = document.querySelector('.telecom-share-answer-dialog');
              if (dialog) {
                const textarea = dialog.querySelector('#telecom-share-answer-json');
                if (textarea) {
                  const updatedInviteJson = JSON.stringify(inviteWithAnswer, null, 2);
                  textarea.value = updatedInviteJson;
                  console.log('[TRICKLE-RECIPIENT] ✅ Updated answer in dialog with new candidate');
                }
              }
            } catch (e) {
              // Dialog might not be open yet, ignore
            }
          } else {
            // null candidate means gathering is complete
        ...
        dataChannel.onopen = () => {
          ...
          // Use current webrtcAnswer which includes all candidates collected so far
          if (!answerSent) {
            console.log('[TRICKLE-RECIPIENT] 🚀 Channel opened - attempting to send answer immediately');
            answerSent = sendAnswerThroughChannel(dataChannel, 'outgoing data channel (onopen)', webrtcAnswer);
        ...
        pc.ondatachannel = (event) => {
          ...
          if (window.TelecomChannelManager) {
            ...
            const existingOutgoingChannel = window._telecomDataChannels?.get(inviteForAnswer.fromGuid);
            if (existingOutgoingChannel) {
              console.log('[TRICKLE-RECIPIENT] ⚠️ Incoming channel received, but outgoing channel exists - NOT overwriting');
              setupMessagesChannelHandler(incomingChannel, inviteForAnswer.fromGuid, 'recipient', { overwrite: true });
              return;
            }
            window._telecomDataChannels.set(inviteForAnswer.fromGuid, incomingChannel);
          ...
          incomingChannel.onopen = () => {
            ...
            if (!answerSent) {
              ...
              answerSent = sendAnswerThroughChannel(incomingChannel, 'incoming data channel (from sender, onopen, updated)', webrtcAnswer);
          ...
          if (incomingChannel.readyState === 'open' && !answerSent) {
            ...
            answerSent = sendAnswerThroughChannel(incomingChannel, 'incoming data channel (already open, updated)', webrtcAnswer);
...
async function processWebRTCAnswer(invite, config, storageKey) {
  ...
  try {
    ...
    // For trickle ICE: candidates may be empty initially and added later via data channel
    // For manual answer: candidates should be present in iceCandidates array
    const isTrickleAnswer = invite.webrtcAnswer.trickle === true;
    const hasCandidates = invite.webrtcAnswer.iceCandidates && invite.webrtcAnswer.iceCandidates.length > 0;
    
    console.log('[INIT] 📊 Answer candidate info:', {
      isTrickle: isTrickleAnswer,
    ...
    if (hasCandidates) {
      ...
      if (candidatesAddedSuccessfully === 0 && invite.webrtcAnswer.iceCandidates.length > 0) {
    ...
    } else if (isTrickleAnswer) {
      console.log('[INIT] ℹ️ Trickle answer with no initial candidates - candidates will be added via data channel as they arrive');
    } else {
      console.warn('[INIT] ⚠️ No ICE candidates in answer and not a trickle answer - connection may fail');
      console.warn('[INIT] 💡 If this is a manual answer, ensure it includes iceCandidates array');
    }
    ...
    // 🚨🚨🚨 CRITICAL: Set up ondatachannel handler FIRST, before checking existing channels 🚨🚨🚨
    // This ensures we receive channels that arrive AFTER answer is processed
    // Handler from sendContactInvite might not be set if answer comes via trickle ICE
    console.log('[Telecom] 🚨 CRITICAL: Setting up ondatachannel handler in processWebRTCAnswer for contact:', contactGuid);
    const existingOndatachannel = pc.ondatachannel;
    pc.ondatachannel = (event) => {
      const incomingChannel = event.channel;
      console.log('[Telecom] 📥 Incoming data channel event (sender, processWebRTCAnswer):', {
        label: incomingChannel.label,
        contactGuid: contactGuid,
        readyState: incomingChannel.readyState,
        channelId: incomingChannel.id,
      ...
      console.log('[Telecom] 🚨 CRITICAL: ondatachannel FIRED in processWebRTCAnswer!');
      
      // Call existing handler if it exists (from sendContactInvite)
      if (existingOndatachannel) {
        try {
          existingOndatachannel(event);
        } catch (e) {
          console.error('[Telecom] Error in existing ondatachannel handler:', e);
        }
      }
      
      // Also handle the channel ourselves if it's messages channel
      if (incomingChannel.label === 'messages') {
        console.log('[Telecom] 📥 Processing incoming messages channel in processWebRTCAnswer');
        window._telecomDataChannels = window._telecomDataChannels || new Map();
        const existingChannel = window._telecomDataChannels.get(contactGuid);
        console.log('[Telecom] Existing channel:', {
          exists: !!existingChannel,
          readyState: existingChannel?.readyState,
          hasOnMessage: !!existingChannel?.onmessage
        });
        
        // Store incoming channel - THIS REPLACES any outgoing channel
        window._telecomDataChannels.set(contactGuid, incomingChannel);
        console.log('[Telecom] ✅ Stored incoming messages channel in processWebRTCAnswer');
        
        // Set up handler if missing or if existing handler doesn't handle regular messages
        const existingHandler = incomingChannel.onmessage;
        const handlesRegularMessages = existingHandler && 
          existingHandler.toString().includes('messageData.type === \'message\'');
        
        if (!handlesRegularMessages) {
          console.log('[Telecom] ⚠️ Existing handler does not handle regular messages - setting up unified handler');
          setupMessagesChannelHandler(incomingChannel, contactGuid, 'sender', { overwrite: true });
        } else {
          console.log('[Telecom] ✅ Existing handler handles regular messages - keeping it');
        }
      } else if (incomingChannel.label === 'control') {
        console.log('[Telecom] 📥 Processing incoming control channel in processWebRTCAnswer');
        window._telecomControlChannels = window._telecomControlChannels || new Map();
        window._telecomControlChannels.set(contactGuid, incomingChannel);
        if (typeof setupControlChannelHandler === 'function') {
          try {
            setupControlChannelHandler(incomingChannel, contactGuid, pc, config, storageKey);
          } catch (e) {
            console.warn('[Telecom] Error setting up control channel handler:', e);
          }
        }
      }
    };
    ...
    const dataChannel = window._telecomDataChannels?.get(contactGuid);
    console.log('[Telecom] 🔍 Checking for existing data channel in processWebRTCAnswer:', {
      contactGuid: contactGuid,
      hasChannel: !!dataChannel,
      channelReadyState: dataChannel?.readyState,
      channelLabel: dataChannel?.label,
      hasOnMessage: !!dataChannel?.onmessage,
    ...
    if (dataChannel) {
      ...
      console.log('[Telecom] 🔍 Channel analysis:', {
        hasIncomingHandler: hasIncomingHandler,
        readyState: dataChannel.readyState,
        hasOnMessage: !!dataChannel.onmessage,
        handlerPreview: dataChannel.onmessage ? dataChannel.onmessage.toString().substring(0, 100) : 'no handler'
      });
      ...
      console.warn('[Telecom] ⚠️ No data channel found in processWebRTCAnswer - channel may arrive later via ondatachannel');
    }
    ...
    if (incomingMessagesChannel) {
      ...
      // 🚨 CRITICAL: Check if handler already exists from sendContactInvite's ondatachannel
      // If handler exists and handles trickle ICE, we should NOT overwrite it
      const existingHandler = incomingMessagesChannel.onmessage;
      const hasTrickleIceHandler = existingHandler && 
        existingHandler.toString().includes('webrtc-answer') &&
        existingHandler.toString().includes('webrtc-answer-candidate');
      
      if (hasTrickleIceHandler) {
        console.log('[Telecom] ✅ Incoming channel already has handler from sendContactInvite (handles trickle ICE) - NOT overwriting');
        console.log('[Telecom] Handler status:', {
          readyState: incomingMessagesChannel.readyState,
          hasOnMessage: !!incomingMessagesChannel.onmessage,
          handlerType: 'trickle-ICE-handler'
        });
      } else if (!incomingMessagesChannel.onmessage) {
        // 🚨 REFACTORED: Use unified handler setup function ONLY if handler is missing
        // This handles race condition where channel opens before ondatachannel fires
        console.log('[Telecom] ⚠️ Incoming channel has NO handler - setting up using unified function');
        setupMessagesChannelHandler(incomingMessagesChannel, contactGuid, 'sender', { overwrite: true });
      } else {
        console.log('[Telecom] ℹ️ Incoming channel has handler but not trickle-ICE handler - keeping existing handler');
        console.log('[Telecom] Handler status:', {
          readyState: incomingMessagesChannel.readyState,
          hasOnMessage: !!incomingMessagesChannel.onmessage
        });
      ...
      console.warn('[Telecom] ⚠️ No incoming messages channel found in processWebRTCAnswer');
      console.warn('[Telecom] Channel may arrive later via ondatachannel - handler is set up above');
    }
    
    // 🚨 CRITICAL: Also check if there are channels that arrived AFTER we set up ondatachannel handler
    // Wait a bit and check again for channels that might have arrived
    setTimeout(() => {
      const delayedChannel = window._telecomDataChannels?.get(contactGuid);
      if (delayedChannel && delayedChannel !== incomingMessagesChannel && delayedChannel !== outgoingMessagesChannel) {
        console.log('[Telecom] 🔍 Found delayed incoming channel after processWebRTCAnswer');
        console.log('[Telecom] Channel details:', {
          readyState: delayedChannel.readyState,
          label: delayedChannel.label,
          hasOnMessage: !!delayedChannel.onmessage
        });
        
        // Check if this is an incoming channel (created by recipient)
        const hasIncomingHandler = delayedChannel.onmessage && 
          delayedChannel.onmessage.toString().includes('Incoming data channel event (sender)');
        
        if (hasIncomingHandler || (!delayedChannel.onmessage && delayedChannel.readyState === 'open')) {
          console.log('[Telecom] ✅ Found incoming channel - setting up handler');
          setupMessagesChannelHandler(delayedChannel, contactGuid, 'sender', { overwrite: true });
        }
      }
    }, 1000);
    
    // Legacy fallback code - will be removed after testing
    if (false && incomingMessagesChannel && incomingMessagesChannel.readyState === 'open' && !incomingMessagesChannel.onmessage) {
    ...
    // Additional check for handler status (outside the disabled fallback block)
    if (incomingMessagesChannel && incomingMessagesChannel.readyState === 'open' && incomingMessagesChannel.onmessage) {
      console.log('[Telecom] ✅ Incoming channel has handler set up correctly');
    } else if (incomingMessagesChannel) {
    ...
    if (outgoingMessagesChannel && outgoingMessagesChannel !== incomingMessagesChannel) {
      ...
      if (outgoingMessagesChannel && !outgoingMessagesChannel.onmessage) {
        ...
        try {
          ...
          const messageData = JSON.parse(event.data);
          console.log('[Telecom] 📥 Parsed message via WebRTC from contact (sender side):', {
            contactGuid: contactGuid,
            type: messageData.type,
            encrypted: messageData.encrypted || false,
            encryptedText: messageData.text ? (messageData.text.substring(0, 100) + (messageData.text.length > 100 ? '...' : '')) : 'no text',
            hasSenderId: !!messageData.senderId,
            hasSenderName: !!messageData.senderName,
          ...
          // Handle incoming WebRTC message
          if (messageData.type === 'message' && messageData.text) {
            console.log('[Telecom] ✅ Processing regular message (sender side):', {
              contactGuid: contactGuid,
              messageLength: messageData.text.length,
              encrypted: messageData.encrypted || false
            });
            ...
            // Determine chatId BEFORE decryption so we can blink immediately after
            const chatId = `contact-${contactGuid}`;
            console.log('[Telecom] 💬 Chat ID:', chatId);
            ...
            const MESSAGES_STORAGE_KEY = `webos.telecom.messages.${chatId}.v1`;
            const messages = getChatMessages(chatId);
            console.log('[Telecom] 💾 Saving message (sender side):', {
              chatId: chatId,
            ...
            messages.push(newMessage);
            localStorage.setItem(MESSAGES_STORAGE_KEY, JSON.stringify(messages));
            console.log('[Telecom] ✅ Message saved (sender side), total messages:', messages.length);
      ...
      } // End of if (!dataChannel.onmessage) block
...
async function sendMessage(win, winId, config, storageKey) {
  console.log('[Telecom] 📤 sendMessage called');
  const messageInput = win.querySelector('#telecom-message-input');
  if (!messageInput) {
    console.warn('[Telecom] ⚠️ Message input not found');
    return;
  }

  const message = messageInput.value.trim();
  console.log('[Telecom] 📝 Message text:', message.substring(0, 100) + (message.length > 100 ? '...' : ''));
  if (!message) {
    console.log('[Telecom] ⚠️ Empty message, ignoring');
    return;
  }

  const selectedChatId = win.dataset.selectedChatId;
  console.log('[Telecom] 💬 Selected chat ID:', selectedChatId);
  if (!selectedChatId) {
    console.warn('[Telecom] ⚠️ No chat selected');
    return;
  }

  // Check if message contains one-tap link (#offer= or #answer=)
  const oneTapHashMatch = message.match(/#(offer|answer)=[^\s]+/);
  console.log('[Telecom] 🔍 Checking for one-tap link in message...', {
    hasMatch: !!oneTapHashMatch,
    matchType: oneTapHashMatch ? oneTapHashMatch[1] : null,
    hashPreview: oneTapHashMatch ? oneTapHashMatch[0].substring(0, 50) + '...' : null
  });
  
  if (oneTapHashMatch) {
    console.log('[Telecom] 🔗 One-tap link detected in message, processing...');
    const hash = oneTapHashMatch[0]; // e.g., "#offer=..."
    console.log('[Telecom] 🔗 Extracted hash:', hash.substring(0, 50) + '...');
    
    // Extract full URL if present, or construct from hash
    let fullUrl = message;
    const urlMatch = message.match(/(https?:\/\/[^\s]+)/);
    if (urlMatch) {
      fullUrl = urlMatch[1];
      console.log('[Telecom] 🔗 Found full URL in message:', fullUrl.substring(0, 100) + '...');
    } else {
      // Construct full URL from hash
      fullUrl = location.origin + location.pathname + hash;
      console.log('[Telecom] 🔗 Constructed URL from hash:', fullUrl.substring(0, 100) + '...');
    }
    ...
    if (window.OneTapTelecom && typeof window.OneTapTelecom.handleIncomingOfferFromUrl === 'function' && typeof window.OneTapTelecom.handleIncomingAnswerFromUrl === 'function') {
      ...
      try {
        ...
        // Extract hash without setting location.hash to avoid triggering hashchange events
        // Process directly using the hash string
        let result = null;
        
        // Process based on hash type - pass hash directly without changing location.hash
        if (hash.startsWith('#offer=')) {
          console.log('[Telecom] 🔄 Processing as offer...');
          // Pass hash directly to avoid triggering hashchange event
          // This prevents double processing
          try {
            result = await window.OneTapTelecom.handleIncomingOfferFromUrl(config, hash);
          } catch (e) {
            console.error('[Telecom] ❌ Error processing offer:', e);
            throw e;
          }
        } else if (hash.startsWith('#answer=')) {
          console.log('[Telecom] 🔄 Processing as answer...');
          // Pass hash directly to avoid triggering hashchange event
          // This prevents double processing
          try {
            result = await window.OneTapTelecom.handleIncomingAnswerFromUrl(config, hash);
          } catch (e) {
            console.error('[Telecom] ❌ Error processing answer:', e);
            throw e;
          }
        }
        
        console.log('[Telecom] 📊 One-tap processing result:', {
          handled: result?.handled,
          hasShareUrl: !!result?.shareUrl,
        ...
        if (result && result.handled) {
          ...
          if (result.shareUrl) {
            ...
            showOneTapLinkDialog(result.shareUrl, 0, 0, winId);
          }
          
          // Don't send the message itself - it was processed as one-tap link
          console.log('[Telecom] ✅ One-tap link processed, clearing input and returning (not sending as regular message)');
          messageInput.value = '';
          messageInput.style.height = 'auto';
          return;
        } else {
      ...
      console.warn('[Telecom] ⚠️ OneTapTelecom handlers not available');
      console.log('[Telecom] Available OneTapTelecom methods:', window.OneTapTelecom ? Object.keys(window.OneTapTelecom) : 'OneTapTelecom is null');
    }
  } else {
    console.log('[Telecom] 📤 No one-tap link found, sending as regular message');
  }
  ...
  console.log('[Telecom] 👤 Peer ID extracted:', peerId);
  
  // 🚨 Use ChannelManager to get correct channel for sending
  ...
  const channelMetadata = window.TelecomChannelManager ? 
    window.TelecomChannelManager.getMetadata(peerId) : null;
  
  console.log('[Telecom] 📡 Data channel status:', {
    hasDataChannel: !!dataChannel,
    readyState: dataChannel ? dataChannel.readyState : null,
    channelLabel: dataChannel ? dataChannel.label : null,
    channelId: dataChannel ? dataChannel.id : null,
    bufferedAmount: dataChannel ? dataChannel.bufferedAmount : null,
    channelMetadata: channelMetadata,
    allChannels: window._telecomDataChannels ? Array.from(window._telecomDataChannels.keys()) : []
  });
  
  if (dataChannel && dataChannel.readyState === 'open') {
    console.log('[Telecom] ✅ Using data channel for sending:', {
      label: dataChannel.label,
      id: dataChannel.id,
      readyState: dataChannel.readyState,
      bufferedAmount: dataChannel.bufferedAmount
    });
    console.log('[Telecom] ✅ Data channel is open, sending message via WebRTC');
    try {
      // Get recipient's public key for encryption
      const contacts = getContacts();
      ...
      const bufferedBefore = dataChannel.bufferedAmount;
      const readyStateBefore = dataChannel.readyState;
      
      // Send message
      dataChannel.send(payloadJson);
      
      // Verify send was successful
      const bufferedAfter = dataChannel.bufferedAmount;
      const readyStateAfter = dataChannel.readyState;
      
      console.log('[Telecom] ✅ Message sent via WebRTC to contact:', peerId);
      console.log('[Telecom] 📊 Data channel state after send:', {
        readyStateBefore: readyStateBefore,
        readyStateAfter: readyStateAfter,
        bufferedBefore: bufferedBefore,
        bufferedAfter: bufferedAfter,
        bufferedIncrease: bufferedAfter - bufferedBefore,
        bufferedAmountLowThreshold: dataChannel.bufferedAmountLowThreshold,
        channelLabel: dataChannel.label,
        channelId: dataChannel.id
      });
      
      // Verify channel is still open and message was buffered
      if (readyStateAfter !== 'open') {
        console.error('[Telecom] ❌ CRITICAL: Channel closed immediately after send!', {
          readyStateBefore,
          readyStateAfter,
          peerId
        });
      }
      if (bufferedAfter <= bufferedBefore && payloadJson.length > 0) {
        console.warn('[Telecom] ⚠️ WARNING: bufferedAmount did not increase after send!', {
          bufferedBefore,
          bufferedAfter,
          payloadLength: payloadJson.length,
          peerId
    ...
    console.warn('[Telecom] ⚠️ Data channel not available or not open:', {
      hasDataChannel: !!dataChannel,
      readyState: dataChannel ? dataChannel.readyState : 'no channel',
      message: 'Message will be saved locally only'
    });
  }
...
(function (global) {
  ...
  OneTapTelecom.ensurePeerForContact = function (contactGuid, config) {
    ...
    if (global._telecomPeerConnections.has(contactGuid)) {
      ...
      cleanIceServers.forEach((server, idx) => {
        ...
        console.log(`[OneTapTelecom] [SENDER] Clean server ${idx}: ${isTurn ? 'TURN' : 'STUN'} - ${server.urls}`, {
          hasUsername: !!server.username,
          hasCredential: !!server.credential,
          credentialsLength: server.username?.length || 0
        });
        if (isTurn && !hasCredentials) {
          console.warn(`[OneTapTelecom] [SENDER] ⚠️ TURN server ${idx} has no credentials - may not work!`);
        }
      });
    ...
    pc.ondatachannel = (e) => {
      ...
      if (ch.label === 'messages') {
        ...
        setupMessagesChannelHandler(ch, contactGuid, 'ensurePeerForContact', { overwrite: true });
  ...
  OneTapTelecom.createOfferLink = async function (contactGuid, config) {
    ...
    try {
      // 🚨 CRITICAL: For reconnect, we need a FRESH PC - close existing PC if it's in failed/closed state or has offer
      // This matches v0.2.2 behavior - always create fresh PC for reconnect
      global._telecomPeerConnections = global._telecomPeerConnections || new Map();
      const existingPC = global._telecomPeerConnections.get(contactGuid);
      
      if (existingPC) {
        const pcState = existingPC.connectionState;
        const signalingState = existingPC.signalingState;
        const iceState = existingPC.iceConnectionState;
        
        // Close PC if it's failed, closed, or has local offer (reconnect needs fresh PC)
        if (pcState === 'failed' || pcState === 'closed' || 
            signalingState === 'have-local-offer' || 
            iceState === 'failed' || iceState === 'disconnected') {
          console.log('[OneTapTelecom] ⚠️ Existing PC needs to be closed for reconnect:', {
          ...
          try {
            ...
            console.warn('[OneTapTelecom] Error closing existing PC:', e);
          }
          ...
          console.log('[OneTapTelecom] ✅ Old PC closed and removed');
        }
      }
      
      console.log('[OneTapTelecom] Ensuring peer connection...');
      // After closing old PC, ensure we get a fresh one
      let pc = OneTapTelecom.ensurePeerForContact(contactGuid, config);
      console.log('[OneTapTelecom] Peer connection ready, state:', pc.connectionState, 'signalingState:', pc.signalingState, 'iceGatheringState:', pc.iceGatheringState);
      
      // 🚨 CRITICAL: Verify PC is fresh and ready for new offer
      // If PC is not in fresh state (stable/new), force close and recreate
      // NOTE: iceGatheringState should be 'new' for fresh PC, 'complete' means it was already used
      if (pc.signalingState !== 'stable' || pc.iceGatheringState !== 'new') {
        console.warn('[OneTapTelecom] ⚠️ PC is not in fresh state! Closing and recreating...', {
          signalingState: pc.signalingState,
          iceGatheringState: pc.iceGatheringState,
          connectionState: pc.connectionState
        });
        ...
        try {
          ...
          console.warn('[OneTapTelecom] Error closing PC:', e);
        }
        global._telecomPeerConnections.delete(contactGuid);
        global._telecomDataChannels?.delete(contactGuid);
        global._telecomControlChannels?.delete(contactGuid);
        ...
        console.log('[OneTapTelecom] ✅ Fresh PC created:', {
          signalingState: pc.signalingState,
          iceGatheringState: pc.iceGatheringState,
          connectionState: pc.connectionState
        });
      ...
      // CRITICAL: Include timestamp to check if offer is still valid (max 5 minutes)
      try {
        ...
        const offerData = {
          ...
          createdAt: new Date().toISOString(),
          timestamp: Date.now(), // Unix timestamp for easy comparison
          contactGuid: contactGuid
        };
        localStorage.setItem(OFFER_STORAGE_KEY, JSON.stringify(offerData));
        console.log('[OneTapTelecom] ✅ Offer SDP saved to localStorage with timestamp:', offerData.timestamp);
      } catch (e) {
   ...
   */
  OneTapTelecom.handleIncomingOfferFromUrl = async function (config, providedHash = null) {
    console.log('[OneTapTelecom] handleIncomingOfferFromUrl called');
    // Use provided hash if available, otherwise read from location.hash
    const hash = providedHash || location.hash || '';
    console.log('[OneTapTelecom] Hash:', hash.substring(0, 50) + '...', 'provided:', !!providedHash);
    if (!hash.startsWith('#offer=')) {
    ...
    try {
      ...
      console.log('[OneTapTelecom] Processing incoming offer - ALWAYS creating fresh PC directly');
      
      // CRITICAL: For processing incoming offer, we MUST create PC DIRECTLY, not via ensurePeerForContact
      // ensurePeerForContact might return existing PC from Map even after delete (race condition or closure issue)
      // Creating PC directly guarantees it's fresh and in 'new' state
      global._telecomPeerConnections = global._telecomPeerConnections || new Map();
      const existingPC = global._telecomPeerConnections.get(contactGuid);
      
      // Close and remove existing PC if it exists (regardless of state)
      if (existingPC) {
        console.log('[OneTapTelecom] Closing existing PC (state:', existingPC.signalingState, ') - always create fresh for incoming offer');
        try {
          existingPC.close();
        } catch (e) {
          console.warn('[OneTapTelecom] Error closing existing PC:', e);
      ...
      cleanIceServers.forEach((server, idx) => {
        ...
        if (isTurn && !hasCredentials) {
          console.warn(`[OneTapTelecom] [RECIPIENT] ⚠️ TURN server ${idx} has no credentials - may not work!`);
        }
      });
      ...
      console.log('[OneTapTelecom] ✅ Fresh PC created DIRECTLY, initial signalingState:', pc.signalingState, '(expected: stable)');
      
      // Set up ondatachannel handler (same as in ensurePeerForContact)
      pc.ondatachannel = (e) => {
        const ch = e.channel;
        global._telecomDataChannels = global._telecomDataChannels || new Map();
        global._telecomControlChannels = global._telecomControlChannels || new Map();
      ...
      // Store in map
      global._telecomPeerConnections.set(contactGuid, pc);
      console.log('[OneTapTelecom] ✅ Fresh PC stored in map, signalingState:', pc.signalingState);

      // 🚨🚨🚨 CRITICAL: Create data channels on recipient side BEFORE processing offer 🚨🚨🚨
      // Recipient MUST create outgoing channels so sender can receive messages via incoming channels
      // Without this, sender has no way to receive messages from recipient!
      console.log('[OneTapTelecom] [RECIPIENT] 🚨 CRITICAL: Creating data channels for bidirectional communication...');
      global._telecomDataChannels = global._telecomDataChannels || new Map();
      global._telecomControlChannels = global._telecomControlChannels || new Map();
      
      // Create messages channel (outgoing from recipient = incoming for sender)
      if (!global._telecomDataChannels.has(contactGuid)) {
        ...
        dcMessages.onclose = () => console.log('[OneTapTelecom] [RECIPIENT] Messages channel closed');
        dcMessages.onerror = (err) => console.warn('[OneTapTelecom] [RECIPIENT] Messages channel error:', err);
      }
      
      // Create control channel (outgoing from recipient = incoming for sender)
      if (!global._telecomControlChannels.has(contactGuid)) {
        console.log('[OneTapTelecom] [RECIPIENT] Creating control data channel...');
        const dcControl = pc.createDataChannel('control', { ordered: true });
        global._telecomControlChannels.set(contactGuid, dcControl);
        if (typeof setupControlChannelHandler === 'function') {
          try {
            ...
            setupControlChannelHandler(dcControl, contactGuid, pc, telecomConfig, 'webos.telecom.v1');
          } catch (e) {
            console.warn('[OneTapTelecom] Error setting up control channel handler:', e);
          }
        }
        dcControl.onopen = () => console.log('[OneTapTelecom] [RECIPIENT] ✅ Control channel opened (outgoing from recipient)');
        dcControl.onclose = () => console.log('[OneTapTelecom] [RECIPIENT] Control channel closed');
        dcControl.onerror = (err) => console.warn('[OneTapTelecom] [RECIPIENT] Control channel error:', err);
      }
      
      console.log('[OneTapTelecom] [RECIPIENT] ✅ Data channels created - recipient can now send messages to sender');

      // 🚨 CRITICAL: Set up ICE candidate handler BEFORE processing offer/answer
      // This ensures we don't miss any candidates during ICE gathering
      const localCandidatesRecipient = [];
      ...
      pc.onicecandidate = (event) => {
        if (event.candidate) {
          const candidateStr = event.candidate.candidate || '';
          if (!candidateStr || candidateStr.trim() === '') {
            console.warn('[OneTapTelecom] [RECIPIENT] ⚠️ Empty candidate string received!');
            return;
          }
          
          // Parse candidate type more carefully
          let type = 'unknown';
          ...
          console.log(`[OneTapTelecom] [RECIPIENT] Local ICE candidate (${type}):`, candidateStr.substring(0, 150));
          
          // Log TURN server info if relay candidate
          if (type === 'relay' && event.candidate.url) {
            console.log(`[OneTapTelecom] [RECIPIENT] ✅ TURN relay candidate from: ${event.candidate.url}`);
          }
        } else {
          ...
          console.log('[OneTapTelecom] [RECIPIENT] ICE candidate summary:', {
            ...
            unknown: candidateTypesRecipient.unknown || 0
          });
          if (candidateTypesRecipient.relay === 0) {
            ...
            console.warn('[OneTapTelecom] [RECIPIENT] 💡 Check: 1) TURN server credentials are correct, 2) TURN server is accessible, 3) Network allows TURN traffic');
          }
        }
      };
      
      // Set up ICE connection state handler BEFORE processing offer/answer
      pc.oniceconnectionstatechange = () => {
      ...
      // Set up connection state handler BEFORE processing offer/answer
      pc.onconnectionstatechange = () => {
      ...
      // Get recipient's current account data (for sending back to sender)
      // This ensures sender gets updated display name, username, etc.
      const systemAccount = window.Auth ? window.Auth.getAccount() : null;
      const recipientDisplayName = config.firstName && config.lastName 
        ? `${config.firstName} ${config.lastName}` 
        : config.username || systemAccount?.username || null;
      ...
      const res = await global.OneTap.handleIncomingOfferAndProduceAnswer(pc, {
        ...
        gzip: true,
        autoShare: true,
        hash: providedHash || hash, // Pass hash explicitly to avoid reading from location.hash
        answerMeta: { 
          ...
          // Include recipient's current data so sender gets updated display name, etc.
          displayName: recipientDisplayName,
          username: recipientUsername,
          firstName: recipientFirstName,
          lastName: recipientLastName,
      ...
      if (res.handled && res.shareUrl) {
        ...
        try {
          ...
          if (effectiveGuid) {
            ...
            if (recipientInvitesData) {
              ...
              // Always ensure contact and chat exist, regardless of invite status
              // This handles reconnection scenarios where invite might be accepted or missing
              if (correctContactGuid) {
                // Update invite status to accepted if invite exists and is pending
                if (invite && invite.status === 'pending') {
                  ...
                  console.log('[OneTapTelecom] [RECIPIENT] Invite already accepted, ensuring contact/chat exist for reconnection');
                } else {
                  console.log('[OneTapTelecom] [RECIPIENT] No pending invite found for contactGuid:', contactGuid, '- ensuring contact/chat exist anyway');
                }
                
                // Always add/update contact (use correctContactGuid from invite.fromGuid - sender's GUID)
                const contacts = getContacts();
                const existingContact = contacts.find(c => c.guid === correctContactGuid);
                if (!existingContact) {
                  // Use invite data if available, otherwise minimal data
                  const newContact = {
                  ...
                  if (invite) {
                    ...
                    if (!existingContact.username && invite.fromUsername) {
                      existingContact.username = invite.fromUsername;
                      updated = true;
                    }
                    if (!existingContact.displayName && invite.fromDisplayName) {
                      existingContact.displayName = invite.fromDisplayName;
                      updated = true;
                    }
                    if (!existingContact.firstName && invite.fromFirstName) {
                      existingContact.firstName = invite.fromFirstName;
                      updated = true;
                    }
                    if (!existingContact.lastName && invite.fromLastName) {
                      existingContact.lastName = invite.fromLastName;
                      updated = true;
                    }
                    if (!existingContact.email && invite.fromEmail) {
                      existingContact.email = invite.fromEmail;
                      updated = true;
                    }
                    if (updated) {
                      saveContacts(contacts);
                      console.log('[OneTapTelecom] [RECIPIENT] ✅ Updated existing contact:', correctContactGuid);
                    }
                  }
                }
                
                // Always create chat if not exists (use correctContactGuid from invite.fromGuid - sender's GUID)
                const chats = getChats();
                ...
                if (!existingChat) {
                  ...
                  console.log('[OneTapTelecom] [RECIPIENT] Chat already exists:', chatId);
                }
              }
            }
          }
        ...
        try {
          ...
          // NOTE: ICE candidate and connection state handlers are set up BEFORE handleIncomingOfferAndProduceAnswer
          // This ensures we don't miss any candidates during ICE gathering
          // Handlers are set up above, right after PC creation
          
          // 🚨🚨🚨 CRITICAL: Set up message handler for messages channel 🚨🚨🚨
          // This must work symmetrically with sender side - same logic for both sides
          if (messagesChannel) {
            console.log('[OneTapTelecom] [RECIPIENT] Messages channel found, state:', messagesChannel.readyState, 'hasHandler:', !!messagesChannel.onmessage);
            ...
            if (messagesChannel.readyState === 'open' && !messagesChannel.onmessage) {
              ...
              messagesChannel.onmessage = async (event) => {
                try {
                  const messageData = JSON.parse(event.data);
                  console.log('[OneTapTelecom] [RECIPIENT] 📥 Received message via channel (already open handler setup):', {
                    contactGuid: contactGuid,
                    type: messageData.type,
                    encrypted: messageData.encrypted || false
                  });
              ...
              console.log('[OneTapTelecom] [RECIPIENT] ✅ Handler set up for already-open channel');
            }
            
            // 🚨 REFACTORED: Use unified handler setup function
            // Note: Reconnect link detection is handled in the unified function
            setupMessagesChannelHandler(messagesChannel, contactGuid, 'recipient', {
              overwrite: true,
              onOpenCallback: () => {
                updateConnectionStatusForChat(contactGuid, 'connected');
              }
            ...
            // Previous handler might be wrong or missing
            // We MUST overwrite it to ensure messages are received
            // Only set up handler if channel is not already open (if open, handler was set above)
            if (messagesChannel.readyState !== 'open') {
              ...
              console.log('[OneTapTelecom] [RECIPIENT] ✅ Message handler set up for messages channel (will activate when channel opens)');
            } else {
              console.log('[OneTapTelecom] [RECIPIENT] Channel already open, handler was set up above');
            }
          } else {
            console.warn('[OneTapTelecom] [RECIPIENT] ⚠️ Messages channel not found!');
            console.warn('[OneTapTelecom] [RECIPIENT] Channel will be created when connection establishes');
          }
   ...
   */
  OneTapTelecom.handleIncomingAnswerFromUrl = async function (config, providedHash = null) {
    ...
    try {
      ...
      try {
        ...
        if (data && data.meta) {
          ...
          console.log('[OneTapTelecom] Answer metadata extracted:', { 
            fromGuid, 
            toGuid,
            ...
            email: data.meta.email ? 'present' : 'missing',
            publicKey: data.meta.publicKey ? 'present' : 'missing'
          });
        } else {
      ...
      console.log('[OneTapTelecom] Peer connection ready, state:', pc.signalingState, {
        ...
        localDescriptionSdpLength: pc.localDescription?.sdp?.length
      });
      ...
      // This matches the logic from v0.2.2 that worked correctly
      ...
      if (pc.signalingState !== 'have-local-offer') {
        ...
        try {
          ...
          if (offerDataStr) {
            ...
            // CRITICAL: Check if offer is still valid (not older than 5 minutes)
            const OFFER_MAX_AGE_MS = 5 * 60 * 1000; // 5 minutes
            const offerAge = offerData.timestamp ? (Date.now() - offerData.timestamp) : Infinity;
            
            if (offerAge > OFFER_MAX_AGE_MS) {
              console.warn('[OneTapTelecom] ⚠️ Saved offer is too old:', {
                ageMs: offerAge,
                ageMinutes: Math.round(offerAge / 60000),
                maxAgeMs: OFFER_MAX_AGE_MS,
                createdAt: offerData.createdAt
              });
              console.warn('[OneTapTelecom] 💡 Offer expired - user needs to create a new reconnect link');
              // Don't restore expired offer - return error to force new reconnect
              return { handled: true, error: 'Saved offer has expired. Please create a new reconnect link.' };
            }
            ...
            console.log('[OneTapTelecom] ✅ Found saved offer, restoring...', {
              ...
              ageMs: offerAge,
              ageSeconds: Math.round(offerAge / 1000)
            });
            ...
            // CRITICAL: When restoring offer from localStorage, we're using the EXACT SDP that was saved
            // This SDP includes ICE candidates that were gathered when the offer was created
            // The answer was created for THIS specific offer, so the ICE candidates should match
            console.log('[OneTapTelecom] Setting local description from saved offer...');
            console.log('[OneTapTelecom] Offer SDP length:', offerData.sdp?.length, 'type:', offerData.type);
            ...
            try {
              ...
              // This matches v0.2.2 logic - simple 3 second wait
              ...
              console.error('[OneTapTelecom] ❌ Error setting local description from saved offer:', e);
              throw e;
            }
          } else {
      ...
      // Extract recipient data from answer token metadata BEFORE processing answer
      // This way we have the data even if answer processing fails
      let recipientDataFromAnswer = null;
      try {
        const tokenB64 = hash.slice('#answer='.length);
        const answerData = await global.OneTap.unpack ? 
          await global.OneTap.unpack(tokenB64, { gzip: true }) :
          null;
        if (answerData && answerData.meta) {
          recipientDataFromAnswer = {
            displayName: answerData.meta.displayName || null,
            username: answerData.meta.username || null,
            firstName: answerData.meta.firstName || null,
            lastName: answerData.meta.lastName || null,
            email: answerData.meta.email || null,
            publicKey: answerData.meta.publicKey || null
          };
          console.log('[OneTapTelecom] [SENDER] Recipient data extracted from answer metadata:', {
            displayName: recipientDataFromAnswer.displayName,
            username: recipientDataFromAnswer.username,
            firstName: recipientDataFromAnswer.firstName,
            lastName: recipientDataFromAnswer.lastName,
            email: recipientDataFromAnswer.email ? 'present' : 'missing',
            publicKey: recipientDataFromAnswer.publicKey ? 'present' : 'missing'
          });
        }
      } catch (e) {
        console.warn('[OneTapTelecom] Error extracting recipient data from answer token:', e);
      }
      
      // Handle answer
      // IMPORTANT: Pass hash explicitly to avoid reading from location.hash (which might be empty or wrong)
      const res = await global.OneTap.handleIncomingAnswer(pc, { 
        gzip: true,
        hash: providedHash || hash // Pass hash explicitly
      });

      console.log('[OneTapTelecom] handleIncomingAnswer result:', res);
      
      // Also try to get data from result (fallback)
      if (!recipientDataFromAnswer && res && res.answerMeta) {
        recipientDataFromAnswer = {
          displayName: res.answerMeta.displayName || null,
          username: res.answerMeta.username || null,
          firstName: res.answerMeta.firstName || null,
          lastName: res.answerMeta.lastName || null,
          email: res.answerMeta.email || null,
          publicKey: res.answerMeta.publicKey || null
        };
        console.log('[OneTapTelecom] [SENDER] Recipient data from answer result:', {
          displayName: recipientDataFromAnswer.displayName,
          username: recipientDataFromAnswer.username
        });
      }
      ...
      if (res.handled && !res.error) {
        ...
        // 🚨🚨🚨 CRITICAL: ALWAYS set up ondatachannel handler for incoming messages channel 🚨🚨🚨
        // This is essential for receiving messages from recipient
        // Even if handler exists from ensurePeerForContact, we MUST overwrite it with sender-specific handler
        // This handler MUST be set up AFTER answer is processed, because answer establishes the connection
        // and triggers ondatachannel events for incoming channels from recipient
        console.log('[OneTapTelecom] [SENDER] 🚨 CRITICAL: Setting up ondatachannel handler for incoming channels...');
        console.log('[OneTapTelecom] [SENDER] This handler will receive messages channel from recipient');
        
        // CRITICAL: Overwrite ondatachannel handler - don't check if it exists, just overwrite it
        // The previous handler (if any) was for recipient side, we need sender side handler
        pc.ondatachannel = (event) => {
            const incomingChannel = event.channel;
            console.log('[OneTapTelecom] [SENDER] 📥 Incoming data channel:', {
              label: incomingChannel.label,
              contactGuid: contactGuid,
              readyState: incomingChannel.readyState
            });
            ...
            if (incomingChannel.label === 'messages') {
              ...
              global._telecomControlChannels = global._telecomControlChannels || new Map();
              global._telecomControlChannels.set(contactGuid, incomingChannel);
              if (typeof setupControlChannelHandler === 'function') {
                try {
                  setupControlChannelHandler(incomingChannel, contactGuid, pc, config, 'webos.telecom.v1');
                } catch (e) {
                  console.warn('[OneTapTelecom] [SENDER] Error setting up control channel:', e);
                }
              }
            }
          };
        ...
        const existingMessagesChannel = global._telecomDataChannels?.get(contactGuid);
        if (existingMessagesChannel) {
          console.log('[OneTapTelecom] [SENDER] Found existing messages channel, setting up handler...');
          setupMessagesChannelHandler(existingMessagesChannel, contactGuid, 'sender', { overwrite: true });
        } else {
          console.log('[OneTapTelecom] [SENDER] ⏳ No messages channel found yet - will be set up when channel arrives via ondatachannel');
        }
        ...
        try {
          ...
          if (effectiveGuid) {
            ...
            if (sentInvitesData) {
              ...
              // Find invite by toGuid (recipient's GUID) - try both pending and accepted
              // This handles reconnection scenarios where invite might already be accepted
              ...
              if (!foundInvite && contactGuid) {
                ...
                // If not found, try accepted (for reconnection)
                if (!foundInvite) {
                  foundInvite = sentInvites.find(inv => inv.toGuid === contactGuid && inv.status === 'accepted');
                }
              }
              ...
              if (correctContactGuid) {
                ...
                if (foundInvite && foundInvite.status === 'pending') {
                  console.log('[OneTapTelecom] [SENDER] Found pending invite, accepting it...');
                  console.log('[OneTapTelecom] [SENDER] Using correct contactGuid from invite.toGuid:', correctContactGuid);
                ...
                } else if (foundInvite && foundInvite.status === 'accepted') {
                  console.log('[OneTapTelecom] [SENDER] Invite already accepted, ensuring contact/chat exist for reconnection');
                } else {
                  console.log('[OneTapTelecom] [SENDER] No invite found for contactGuid:', contactGuid, '- ensuring contact/chat exist anyway');
                }
                
                // Always add/update contact (use correctContactGuid from invite.toGuid)
                // Priority: answerMeta (most recent) > invite data > existing contact data
                const contacts = getContacts();
                const existingContact = contacts.find(c => c.guid === correctContactGuid);
                if (!existingContact) {
                  // Use answerMeta data first (most recent), then invite data, then minimal data
                  const newContact = {
                    guid: correctContactGuid,
                    username: recipientDataFromAnswer?.username || foundInvite?.toUsername || null,
                    displayName: recipientDataFromAnswer?.displayName || foundInvite?.toDisplayName || foundInvite?.toUsername || correctContactGuid.substring(0, 8) + '...',
                    firstName: recipientDataFromAnswer?.firstName || foundInvite?.toFirstName || null,
                    lastName: recipientDataFromAnswer?.lastName || foundInvite?.toLastName || null,
                    email: recipientDataFromAnswer?.email || foundInvite?.toEmail || null,
                    publicKey: recipientDataFromAnswer?.publicKey || foundInvite?.toPublicKey || null,
                    addedAt: new Date().toISOString()
                  };
                  contacts.push(newContact);
                  saveContacts(contacts);
                  console.log('[OneTapTelecom] [SENDER] ✅ Contact added:', correctContactGuid, 'displayName:', newContact.displayName);
                } else {
                  console.log('[OneTapTelecom] [SENDER] Contact already exists:', correctContactGuid);
                  // Update existing contact with data from answerMeta (most recent) or invite
                  // Always update if answerMeta has data (it's the most recent)
                  let updated = false;
                  
                  // Update from answerMeta (most recent data from recipient)
                  if (recipientDataFromAnswer) {
                    if (recipientDataFromAnswer.displayName && recipientDataFromAnswer.displayName !== existingContact.displayName) {
                      existingContact.displayName = recipientDataFromAnswer.displayName;
                      updated = true;
                    }
                    if (recipientDataFromAnswer.username && recipientDataFromAnswer.username !== existingContact.username) {
                      existingContact.username = recipientDataFromAnswer.username;
                      updated = true;
                    }
                    if (recipientDataFromAnswer.firstName && recipientDataFromAnswer.firstName !== existingContact.firstName) {
                      existingContact.firstName = recipientDataFromAnswer.firstName;
                      updated = true;
                    }
                    if (recipientDataFromAnswer.lastName && recipientDataFromAnswer.lastName !== existingContact.lastName) {
                      existingContact.lastName = recipientDataFromAnswer.lastName;
                      updated = true;
                    }
                    if (recipientDataFromAnswer.email && recipientDataFromAnswer.email !== existingContact.email) {
                      existingContact.email = recipientDataFromAnswer.email;
                      updated = true;
                    }
                    if (recipientDataFromAnswer.publicKey && recipientDataFromAnswer.publicKey !== existingContact.publicKey) {
                      existingContact.publicKey = recipientDataFromAnswer.publicKey;
                      updated = true;
                    }
                  }
                  
                  // Update from invite if missing (fallback)
                  if (foundInvite) {
                    if (!existingContact.publicKey && foundInvite.toPublicKey) {
                      existingContact.publicKey = foundInvite.toPublicKey;
                      updated = true;
                    }
                    if (!existingContact.username && foundInvite.toUsername) {
                      existingContact.username = foundInvite.toUsername;
                      updated = true;
                    }
                    if (!existingContact.displayName && foundInvite.toDisplayName) {
                      existingContact.displayName = foundInvite.toDisplayName;
                      updated = true;
                    }
                    if (!existingContact.firstName && foundInvite.toFirstName) {
                      existingContact.firstName = foundInvite.toFirstName;
                      updated = true;
                    }
                    if (!existingContact.lastName && foundInvite.toLastName) {
                      existingContact.lastName = foundInvite.toLastName;
                      updated = true;
                    }
                    if (!existingContact.email && foundInvite.toEmail) {
                      existingContact.email = foundInvite.toEmail;
                      updated = true;
                    }
                  }
                  ...
                  if (updated) {
                    saveContacts(contacts);
                    console.log('[OneTapTelecom] [SENDER] ✅ Updated existing contact:', correctContactGuid, 'displayName:', existingContact.displayName);
                  }
                }
                
                // Always create chat if not exists (use correctContactGuid from invite.toGuid)
                // Update chat name if displayName changed
                const chats = getChats();
                const chatId = `contact-${correctContactGuid}`;
                const existingChat = chats.find(c => c.id === chatId);
                const newChatName = recipientDataFromAnswer?.displayName || foundInvite?.toDisplayName || foundInvite?.toUsername || correctContactGuid.substring(0, 8) + '...';
                ...
                if (!existingChat) {
                  ...
                  localStorage.setItem('webos.telecom.chats.v1', JSON.stringify(chats));
                  console.log('[OneTapTelecom] [SENDER] ✅ Chat created:', chatId, 'with contact:', correctContactGuid, 'name:', newChatName);
                  ...
                  console.log('[OneTapTelecom] [SENDER] Chat already exists:', chatId);
                  // Update chat name if displayName changed (from answerMeta or invite)
                  if (newChatName && newChatName !== existingChat.name) {
                    existingChat.name = newChatName;
                    localStorage.setItem('webos.telecom.chats.v1', JSON.stringify(chats));
                    console.log('[OneTapTelecom] [SENDER] ✅ Chat name updated:', chatId, 'new name:', newChatName);
        ...
        // Set up connection state monitoring with detailed logging
        pc.onconnectionstatechange = () => {
          ...
          if (pc.connectionState === 'connected') {
            ...
            // CRITICAL: Update UI to show connected status (green circle)
            ...
            console.error('[OneTapTelecom] Diagnostics:', {
              ...
              localDescriptionSdpLength: pc.localDescription?.sdp?.length,
              remoteDescriptionSdpLength: pc.remoteDescription?.sdp?.length
            });
            // CRITICAL: Update UI to show disconnected status (red circle)
            if (typeof updateConnectionStatusForChat === 'function') {
              updateConnectionStatusForChat(contactGuid, 'disconnected');
            }
          }
        ...
        pc.oniceconnectionstatechange = () => {
          ...
          if (pc.iceConnectionState === 'connected' || pc.iceConnectionState === 'completed') {
            ...
            // CRITICAL: Update UI when ICE connects
            if (typeof updateConnectionStatusForChat === 'function' && pc.connectionState === 'connected') {
              updateConnectionStatusForChat(contactGuid, 'connected');
            }
          } else if (pc.iceConnectionState === 'checking') {
            ...
            console.error('[OneTapTelecom] ❌ ICE connection failed - possible causes:');
            console.error('[OneTapTelecom] 1. NAT/firewall blocking peer-to-peer connection');
            console.error('[OneTapTelecom] 2. TURN server not configured or not working');
            console.error('[OneTapTelecom] 3. Offer/answer mismatch (different ICE candidates)');
            // CRITICAL: Update UI to show disconnected status
            if (typeof updateConnectionStatusForChat === 'function') {
              updateConnectionStatusForChat(contactGuid, 'disconnected');
            }
          }
        ...
        const localCandidates = [];
        const candidateTypes = { host: 0, srflx: 0, relay: 0, prflx: 0, other: 0, unknown: 0 };
        pc.onicecandidate = (event) => {
          if (event.candidate) {
            const candidateStr = event.candidate.candidate;
            // Parse candidate type more carefully
            ...
            console.log(`[OneTapTelecom] [SENDER] Local ICE candidate (${type}):`, candidateStr.substring(0, 150));
            
            // Log TURN server info if relay candidate
            if (type === 'relay' && event.candidate.url) {
              console.log(`[OneTapTelecom] [SENDER] ✅ TURN relay candidate from: ${event.candidate.url}`);
            }
          } else {
            ...
            console.log('[OneTapTelecom] [SENDER] ICE candidate summary:', {
              ...
              unknown: candidateTypes.unknown
            });
            if (candidateTypes.relay === 0) {
              ...
              console.warn('[OneTapTelecom] [SENDER] 💡 Check: 1) TURN server credentials are correct, 2) TURN server is accessible, 3) Network allows TURN traffic');
            }
          }
        };
        
        // 🚨🚨🚨 CRITICAL: Double-check that handler is set up on already-open channels 🚨🚨🚨
        // After setting up ondatachannel handler, check again if channel opened in the meantime
        // This is a safety check to ensure handler is ALWAYS set up
        const messagesChannel = global._telecomDataChannels?.get(contactGuid);
        const controlChannel = global._telecomControlChannels?.get(contactGuid);
        
        // Log channel states and verify handler is set up
        if (messagesChannel) {
          console.log('[OneTapTelecom] [SENDER] Messages channel exists, state:', messagesChannel.readyState, 'hasHandler:', !!messagesChannel.onmessage);
          
          // 🚨 CRITICAL: If channel is open but handler is missing, set it up NOW
          if (messagesChannel.readyState === 'open' && !messagesChannel.onmessage) {
            console.error('[OneTapTelecom] [SENDER] 🚨 CRITICAL: Channel is open but NO handler! Setting up handler NOW...');
            // Use the same handler logic as above
            messagesChannel.onmessage = async (event) => {
              try {
                const messageData = JSON.parse(event.data);
                console.log('[OneTapTelecom] [SENDER] 📥 Received message via channel (late handler setup):', {
                  contactGuid: contactGuid,
                  type: messageData.type,
                  encrypted: messageData.encrypted || false
                });
                ...
                if (messageData.type === 'message' && messageData.text) {
                  ...
                  if (messageData.encrypted) {
                    try {
                      ...
                      console.error('[OneTapTelecom] [SENDER] Error decrypting:', e);
                      decryptedText = '[Encrypted message - decryption failed]';
                    }
                  }
                ...
                console.error('[OneTapTelecom] [SENDER] Error parsing message:', e);
              }
            };
            console.log('[OneTapTelecom] [SENDER] ✅ Handler set up on already-open channel');
          }
          
          // 🚨 REFACTORED: Use unified handler setup function
          if (messagesChannel.readyState === 'open') {
            console.log('[OneTapTelecom] [SENDER] ✅ Messages channel is already open - setting up handler');
            setupMessagesChannelHandler(messagesChannel, contactGuid, 'sender', { overwrite: true });
            if (typeof updateConnectionStatusForChat === 'function') {
              updateConnectionStatusForChat(contactGuid, 'connected');
            }
          } else {
            console.log('[OneTapTelecom] [SENDER] ⏳ Messages channel not yet open, setting up handler for when it opens');
            setupMessagesChannelHandler(messagesChannel, contactGuid, 'sender', { 
              overwrite: true,
              onOpenCallback: () => {
                if (typeof updateConnectionStatusForChat === 'function') {
                  updateConnectionStatusForChat(contactGuid, 'connected');
          ...
          console.warn('[OneTapTelecom] [SENDER] ⚠️ No messages channel found - will be created when connection establishes');
        }
        
        if (controlChannel) {
          console.log('[OneTapTelecom] [SENDER] Control channel exists, state:', controlChannel.readyState);
          if (controlChannel.readyState === 'open') {
            console.log('[OneTapTelecom] [SENDER] ✅ Control channel is already open');
          }
        }
        
        // NOTE: Message handlers are already set up above:
        // 1. Via ondatachannel handler (for incoming channels) - line 15228
        // 2. Via existing channel check (if channel already open) - line 15363
        // Additional onopen/onclose handlers are set up above if needed
        ...
        // CRITICAL: Verify that connection can actually send/receive messages
        // Wait a bit for connection to establish, then verify channels are open
        setTimeout(() => {
          const finalMessagesChannel = global._telecomDataChannels?.get(contactGuid);
          const finalControlChannel = global._telecomControlChannels?.get(contactGuid);
          console.log('[OneTapTelecom] 🔍 Final connection check after answer processing:', {
            connectionState: pc.connectionState,
            iceConnectionState: pc.iceConnectionState,
            signalingState: pc.signalingState,
            messagesChannelState: finalMessagesChannel?.readyState,
            controlChannelState: finalControlChannel?.readyState,
            canSendMessages: finalMessagesChannel?.readyState === 'open'
          });
          
          if (pc.connectionState === 'connected' && finalMessagesChannel?.readyState === 'open') {
            console.log('[OneTapTelecom] ✅ Connection fully established - messages can be sent/received');
          } else {
            console.warn('[OneTapTelecom] ⚠️ Connection not fully established yet:', {
              connectionState: pc.connectionState,
              messagesChannelReady: finalMessagesChannel?.readyState === 'open'
            });
          }
        }, 2000);
